...
//pop-up notification whenever add/remove from cart + others 
...
export const StateContext = ({ children }) => {
  ...
  const toggleCartItemQuantity = (id, value) => {
    ...
    //use filter method because it is non-mutative (does not mutate State directly) 
    const newCartItems = cartItems.filter((item) => item._id !== id)

    if(value === 'inc') {
      //updating newCartItems adding a new product 
      //spread the properties of the new product and increasing qty by 1
...
//allows use of state like a hook 
